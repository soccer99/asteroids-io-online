  ...
    ws.onmessage = (e) => {
    console.log("ws.onmessage")
      ...
      if (data.type == "INITIAL_STATE") {
        ...
        enemyShips[new_player_id] = this.physics.add.image(new_player.x, new_player.y, 'ship');
    ...
    for (let other_ship_id in enemyAcceleration) {
      if (other_ship_id != player_id) {
        if (enemyAcceleration[other_ship_id] == true) {
          this.physics.velocityFromRotation(enemyShips[other_ship_id].rotation, 200, enemyShips[other_ship_id].body.acceleration);
        } else {
